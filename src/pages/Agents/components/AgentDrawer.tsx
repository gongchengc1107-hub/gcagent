...
import { Drawer, Form, Input, Select, Switch, Button, Space, Checkbox } from 'antd'
...
const AgentDrawer: FC<AgentDrawerProps> = ({
  ...
  useEffect(() => {
    ...
    if (mode === 'edit' && agent) {
      // tools: Record<string, boolean> → 勾选的工具 value[]
      ...
      form.setFieldsValue({
        ...
        autoMode: false,
        skillIds: [],
        tools: ALL_TOOLS.map((t) => t.value), // 默认全选
        systemPrompt: '',
      })
    }
  ...
  const handleFinish = (values: Record<string, unknown>) => {
    // 创建时校验 backendName 不与内置 agent 冲突
    if (mode === 'create') {
